...
1. 有料プランを追加：広告削除 + 対戦相手の広告削除、追加のAIを利用、英語以外のユーザー名も設定可能、優先マッチング、リバーシ検定の資格授与、オンライン対戦のルール決定の優先？
1. オンラインマッチ機能（レート別）の実装 + 誰もいない時のためのAIボット +  負けそうになると接続を切る人がいるので通信切れに罰則を + ブラックリスト実装/ 制限時間を調整して6分対局と3分対局をつくる
1. adsense折りたたみ広告の削除を検討
